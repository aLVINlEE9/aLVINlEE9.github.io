...
그런데 일반적으로 특정 시간 **t**에서의 상태 **S<sub>t</sub>** 는 정해져있지 않을것이다. ( (1, 1), (5, 5) 등...).
...
(3,1)또는 (4,1)이라는 state 를  특정 state이기 때문에 우리는 소문자로 나타낼 수 있다고 말했다.

따라서 **s<sub>1</sub> = (3,1)** 이라고 하고 **s<sub>2</sub> = (4,1)**이라고 한다면,

**S<sub>t</sub> = s<sub>1</sub>** 에서 **A<sub>t</sub> = right** 라는 행동을 한 후 **S<sub>t+1</sub> = s<sub>2</sub>** 으로 이동 하였다고 할 수 있다.
